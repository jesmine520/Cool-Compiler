...
void program_class::cgen(ostream &os) 
{
  ...
  codegen_classtable = new CgenClassTable(classes, os); // 给全局变量赋值
...
// =========================
// cgen.cc：整段替换 emit_push
// 统一约定：push 后栈顶是 0($sp)
// =========================
static void emit_push(char *reg, ostream& str)
{
  // 标准约定：先移动 sp，再把值放到 0($sp)
  emit_store(reg, 0, SP, str); 
  emit_addiu(SP, SP, -4, str);
 // store 到 4($sp) == 0($sp) 的槽位
}
...
//-------------------上面对应我的CgenClassTable::code_class_inits-----------------//
//-------------------------结束--------------------//
...
//---------------------之前自己写的版本-------------------//
            ...
            //   codegen_classtable = this;   // ✅ 关键：构造期间也要可用
...
void assign_class::code(ostream &s, Environment env) {
    // 1. 首先计算右侧表达式的值，结果存入 $a0
    s << "\t# Assign: evaluating expr" << endl;
    expr->code(s, env);

    int idx;
    // 2. 依次在局部变量、形参、属性中查找左值
    if ((idx = env.LookUpVar(name)) != -1) {
        s << "\t# Assign to let variable: " << name << endl;
        emit_store(ACC, idx + 1, SP, s); // 关键：idx + 1 匹配标准压栈约定
                if (cgen_Memmgr == 1) {
        ...
        s << "\t# Assign to parameter: " << name << endl;
        emit_store(ACC, idx + 3, FP, s); // 关键：idx + 3 匹配标准栈帧
                if (cgen_Memmgr == 1) {
...
// cgen.cc：整段替换 static_dispatch_class::code
// cgen.cc：整段替换 static_dispatch_class::code
...
// =========================
// cgen.cc：整段替换 typcase_class::code
// =========================
...
// cgen.cc：整段替换 let_class::code
// cgen.cc：替换 let_class::code
// cgen.cc：整段替换 let_class::code
// =========================
// cgen.cc：整段替换 let_class::code
// =========================
...
// COOL 的 not 运算：comp
// 规则：e1 先算出 Bool 对象放在 ACC，然后取出 Bool.val (0/1)，再返回相反的 Bool 常量对象
...
void no_expr_class::code(ostream &s, Environment env) {
    // no_expr 在 codegen 中一般表示“什么都不生成”，但为了安全让 ACC=0
    emit_move(ACC, ZERO, s);
}
